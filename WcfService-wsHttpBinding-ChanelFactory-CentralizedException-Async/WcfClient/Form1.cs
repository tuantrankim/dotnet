...
namespace WcfClient
{
    public partial class Form1 : Form
    {
        ...
        private async void btnGetEmpAsync_Click(object sender, EventArgs e)
        {
            try
            {
                var emp = await sw.Channel.GetEmployeeAsyn(Convert.ToInt32(txtID.Text));
                if (emp != null)
                {
                    txtName.Text = emp.Name;
                    txtGender.Text = emp.Gender;
        ...
        private async void btnSaveEmpAsync_Click(object sender, EventArgs e)
        {
            try
            {
                var emp = new WCFService.Employee()
                ...
                await sw.Channel.SaveEmployeeAsyn(emp);
                lblMessage.Text = "Save employee successfully.";
            }
            catch (FaultException ex)
            {
